...
class User extends App{	
	...
	protected function login(){
		...
		$this->footer();		
		    ...
		    require(ROOT."/includes/library/auth/facebook.php");  

		    // Creating the facebook object  
		    $facebook = new Facebook(array(  
		        'appId'  => $this->config["facebook_app_id"],
		        'secret' =>  $this->config["facebook_secret"], 
		        'cookie' => true
		    )); 
		    ...
		    if(!empty($session)) { 
		      $fb = $facebook->api('/me');   
		      if(!isset($fb["email"])) return Main::redirect("",array("danger",e("You must grant permission to this application to use your profile information.")));
		      ...
		      if($user=$this->db->get("user","auth='facebook' AND (email='{$fb["email"]}' OR auth_id='{$fb["id"]}')",array("limit"=>1))){
						...
						// Inser AuthID
						if(empty($user->auth_id) && isset($fb["auth_id"])){	
							// Update database
							$this->db->update("user",array("auth_id"=>"?"),array("id"=>$user->id),array($fb["auth_id"]));
						}
		      	...
		      	$data = array(
		      			...
		      			":date" => "NOW()",
		      			":auth" => "facebook",
		      			":auth_id" => isset($fb["id"]) ? Main::clean($fb["id"],3,TRUE) : "",
		      			":api" => Main::strrand(12),
		      			":auth_key" => $auth_key
		      		);
  ...
  private function edit(){
  	...
  	if(empty($this->id) || !is_numeric($this->id)) return Main::redirect("user",array("danger",e("This URL doesn't exist.")));
		if(!$url = $this->db->get("url",array("id"=>"?"),array("limit"=>1),array($this->id))) return Main::redirect("user",array("danger",e("This URL doesn't exist.")));
    ...
    $content="      
	  ...
	  $widgets.='<div class="panel panel-default panel-body" id="'.__FUNCTION__.'">';
				...
				$widgets.="<p><i class='glyphicon glyphicon-link'></i> {$this->user->domain}/{$url->alias}{$url->custom} <a href='#' class='inline-copy copy' data-value='{$this->user->domain}/{$url->alias}{$url->custom}'>".e("Copy")."</a></p>";
				$widgets.="<p><i class='glyphicon glyphicon-qrcode'></i> {$this->user->domain}/{$url->alias}{$url->custom}/qr <a href='#' class='inline-copy copy' data-value='{$this->user->domain}/{$url->alias}{$url->custom}/qr'>".e("Copy")."</a></p>";
    ...
    // Show Template
		...
		$this->header();
		include($this->t("components/user_template"));
	 	$this->footer();	  	
  }
  /**
   * Edit URLs
  ...
  private function splash(){	  	
	...
	protected function settings(){
		...
		if($this->config["multiple_domains"]){
			$domain_list='<div class="form-group"><label class="col-sm-3 control-label">'.e("Default Domain Name").'</label><div class="col-sm-9"><select name="domain" id="domains">';
			$domain_list.='<optgroup label="'.e('Choose Domain').'" />';
			$domains=explode("\n", $this->config["domain_names"]);
			$domain_list.='<option value="" '.(($this->user->domain==$this->config["url"] || empty($this->user->domain))?'selected':'').'>'.ucfirst(str_replace("https://","",str_replace("http://", "",$this->config["url"]))).'</option>';
			foreach ($domains as $domain) {
				$domain_list.='<option value="'.strtolower(rtrim($domain)).'" '.($this->user->domain==strtolower(rtrim($domain))?'selected':'').'>'.ucfirst(str_replace("https://","",str_replace("http://", "", trim($domain)))).'</option>';
			}
		...
		private function last_payments(){
          ...
          foreach ($payments as $payment){
            	$html .='<tr data-id="'.$payment->id.'">
					              ...
					              <td>'.date("d F, Y H:i",strtotime($payment->date)).'</td>
					              <td>'.date("d F, Y H:i",strtotime($payment->expiry)).'</td>
					            </tr>';     
          }
		        $html .='</tbody>
						      </table>';
