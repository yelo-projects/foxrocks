...
class Main{
  ...
  */
    public static function image(){      
      if(empty(self::$image)){
        return "http://s.wordpress.com/mshots/v1/".self::url()."?w=800";
      }else{
        return self::$image;
      }
    }   
  ...
  public static function check_captcha($array){
      if(self::$config["captcha"]=="1"){
        ...
        if(empty($array["recaptcha_response_field"])) {
          ...
          if (!$resp->is_valid) {
            return e("The CAPTCHA wasn't entered correctly. Please try it again.")."<script>Recaptcha.reload()</script>";
          }
        }
...
/**
  ...
  */   
  ...
  public static function curl($url,$option=array()){   
    ...
    if(in_array('curl', get_loaded_extensions())){ 
      ...
      curl_setopt($curl, CURLOPT_USERAGENT, "Premium URL Shortener 4.0");
      $resp = curl_exec($curl);
      curl_close($curl);
      return $resp;
    }
  ...
  * @since 4.1
  **/
 public static function register($area, $fn){
  if(function_exists($fn)) {
    self::$plugin[$area][] = $fn;  
  }
 }  
