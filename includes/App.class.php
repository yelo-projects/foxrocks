...
class App{
	...
	protected $actions=array("user","page","contact","server","upgrade","analytic","profile","ipn");	
	/**
	 * User Variables
	 * @since 4.0
	 **/
	...
	protected function home(){
		...
		if(isset($_GET["r"]) && !empty($_GET["r"])){
			return Main::redirect(base64_decode($_GET["r"]),array(),301,TRUE);
		}
		...
		include(TEMPLATE."/index.php");
		 ...
		 /**
			...
			private function bookmark(){
				...
				require(ROOT."/includes/Short.class.php");
				$short = new Short($this->db,$this->config);  			
				$short=$short->add(array("url"=>$_GET["url"]));		
				...
				if($short["error"]){
					Main::redirect("?url={$_GET["url"]}",array("danger",$short["msg"]));
					return;			
				}else{
					Main::redirect($short["short"]."+",array("success",e("URL has been successfully shortened")),"",TRUE);
					return;
				}
			}
			/**
	      ...
	      $query.=") AND userid=:user";				
			 ...
			 * @since  4.2
			 */
			protected function public_list($limit = 25){
				if(!$this->config["public_dir"]) return FALSE;

				if($urls = $this->db->get("url",array("public" => "1", "userid" => "0"),array("limit"=>25,"order"=>"date"))){
	...
	protected function user(){
		...
		if($this->logged()){
			$action=array("edit","delete","archive","bundles","splash","settings","logout","verify","search","server","export");
		}else{
		...
		$urls=$this->db->get("url",array("userid"=>"?","archived"=>"0"),array("order"=>$order[0],"limit"=>(($this->page-1)*$this->limit).", {$this->limit}","count"=>TRUE,"asc"=>$order[1]),array($this->userid));
	...
	*/
	...
	protected function user_menu($option=array()){
		$menu='<ul class="nav nav-sidebar">';
			...
			$menu.='<li><a href="'.Main::href("profile/{$this->user->username}").'"><span class="glyphicon glyphicon-cloud"></span> '.e('Public Profile').''.$public.'</a></li>';
