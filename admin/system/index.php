...
<div class="row stats">
  ...
  <div class="col-md-3">
    <div class="panel panel-default panel-black">
      <div class="panel-body">
        <p class="main-stats"><span><?php echo Main::currency($this->config["currency"],$this->db->count("payment","(MONTH(date) = MONTH(CURDATE()) AND YEAR(date) = YEAR(CURDATE())) AND status='Completed'","amount")) ?></span> in <?php echo date("F") ?></p>
         <p><?php echo Main::currency($this->config["currency"],$this->db->count("payment","status='Completed'","amount")) ?> Total</p>
      </div>
    </div>
  </div>       
</div><!--/.stats-->
      ...
      <div class="panel-body nopadding">
        <?php foreach ($urls as $url): ?>
          <div class="url-list">
            ...
            <a href="<?php echo $this->config["url"]."/".$url->custom.$url->alias ?>" target="_blank">
              <strong><?php echo $this->config["url"]."/".$url->custom.$url->alias ?></strong>
            </a>
