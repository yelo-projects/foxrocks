...
<html lang="en">
  ...
  <body>
  	<div class="container">
  		...
  		<div class="right">
				<h1>Upgrading Premium URL Shortener to 3.0</h1> 
				<p>
					You are about to upgrade this software to version <strong>3.0</strong>. Please note that this will only update your database and NOT your files. It is strongly recommended that you first backup your database then your existing files in case something unexpected occurs. 
				</p>
				<p>
					Version 3.0 is adds many new functionality including improvements in performance, maintainability and security. For this reason, <strong>a lot of files</strong> were updated. <strong>Please read</strong> the update manual carefully in order to make this update as smoothly as possible. 
				</p>
				...
				<a href="update3.php?step=2" class="button">I am ready, please update my database</a>		
  		</div>  		
  	</div>
  </body>
</html>
...
function get_query($dbinfo){
	...
	('custom_splash_amount',''),
	('show_media','1'),
	('removal_amount',''),
	('paypal_email',''),
	('logo','');";

	$query[]="ALTER TABLE `{$dbinfo["prefix"]}url` ADD `archived` int(1) NOT NULL DEFAULT '0'";
 
	$query[]="ALTER TABLE  `{$dbinfo["prefix"]}user` ADD `domain` varchar(255) NOT NULL";
	$query[]="ALTER TABLE  `{$dbinfo["prefix"]}user` ADD `media` int(1) NOT NULL DEFAULT '1'";
	$query[]="ALTER TABLE  `{$dbinfo["prefix"]}user` ADD `splash_opt` int(1) NOT NULL DEFAULT '0'";
	$query[]="ALTER TABLE  `{$dbinfo["prefix"]}user` ADD `splash` text NOT NULL";
		return $query;
}
